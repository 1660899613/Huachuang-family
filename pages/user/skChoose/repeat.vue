		...
		<view class="content">
			<view class="ul">
				<view class="li">
					...
					<view class="row1">
						<view class="row">
							...
							<view class="input box-1">
								<input :readonly="true" placeholder="请输入学员姓名" type="text" v-model="studentInfor.studentName" />
						...
						<view class="row">
							...
							<view class="input box-1">
								<input :readonly="true" placeholder="请输入学员联系方式" type="text" v-model="studentInfor.studentPhone" />
							</view>
							<view class="icon">
								<u-icon name="arrow-right"></u-icon>
							</view>
					...
					<view class="addBtn btn" @click="submit">确认</view>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods:{
			...
			//查询学员信息
			getClassRetrainInfo(){
				this.$u.api.getClassRetrainInfo({
						classId:this.id,
					}).then(res=>{
					console.log(res,'data')
					this.studentInfor = res.data.info
					// this.infor.coursePackage = 2
				})
			},
			//确认学员信息
			submit(){
				...
				this.$u.api.choseCourseClass({
					...
					if(res.status){
						this.$f.jump('/pages/pay/reOrder?id='+this.orderId)
					}else{
